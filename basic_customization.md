...
This page assumes that Talon is responding to your voice and you are using the `community` Talon user file set.
...
All customization consists of files with `.talon` or `.py` file extensions placed in the Talon user directory. The Talon user directory is where you put the `community` Talon user file set ("~/.talon/user/" on MacOS/Linux, "%APPDATA%\Talon\user" on Windows). Talon doesn't care how you organize your files within this directory, any subdirectories or file names are just there to make things easier to understand for you and others.
...
* Option A: Edit the `.talon` and `.py` files within your `community` repository directory.
* Option B: Maintain your own separate directory with your customizations.

Option A can be easier to begin with, but makes it somewhat harder to keep your `community` scripts up to date. This is because you need to work out what you modified and how to re-apply it to the knausj Talon user file set every time you update it. We recommend using Option B with separate directories as much as possible, only resorting to direct editing when you have to.

The screenshot below shows my Talon user directory. You can see that I have a `community` folder in the same way that you would. The rest of the folders are other 'Talon user file sets' I'm using. All my personal customizations are in the 'mystuff' folder; `community` is completely unchanged from what I downloaded from github.

![Screen shot of Talon user directory](/media/basic_customization_folders.png)

Let's speak about versioning your configuration for a minute. You will probably want to update your `community` Talon user file set occasionally in order to pick up new features and bug fixes. Unfortunately changing `community` may also sometimes add new bugs! If this happens you might want to go back the older version so you can keep working. If you're confident with the `git` program this is straightforward. If you're not, then just making a backup of the whole Talon user directory prior to making a potentially significant change doesn't hurt.
...
Let's make a new voice command that presses the key combination cmd+a or control+a when you say "select everything". This will work in a similar way to the 'select all' command built in to `community`.
...
You can paste the following code into the REPL to see a full list of available settings: `settings.list()`. A list of some of the more useful ones are [included here](/unofficial_talon_docs#built-in-talon-settings). `community` also has a list of some extra settings it defines in the `settings.talon` file.
...
This also provides a simple way of overriding the behaviour of existing voice commands from the `community` Talon user file set. Lets say you wanted to change the behaviour of the `touch` command so that it didn't hide the mouse grid if it was open.
